        ...
        if (nextLineIdx < lines.length) {
          ...
          if (nextIndent > currentIndent) {
            ...
            if (nextContent.startsWith('- ')) {
              const arr: unknown[] = [];
              const endIdx = YamlAccessor.parseArray(lines, nextLineIdx, nextIndent, arr);
              target[key] = arr;
              i = endIdx;
              continue;
            }
            ...
            const endIdx = YamlAccessor.parseBlock(lines, nextLineIdx, nextIndent, nested);
            target[key] = nested;
            i = endIdx;
            continue;
          }
    ...
    while (i < lines.length) {
      ...
      if (colonIdx !== -1 && !itemValue.startsWith('"') && !itemValue.startsWith("'")) {
        ...
        if (val === '') {
          ...
          const nextLineIdx = YamlAccessor.findNextContentLine(lines, i + 1);
          if (nextLineIdx < lines.length) {
            const nextIndent = lines[nextLineIdx].length - lines[nextLineIdx].trimStart().length;
            if (nextIndent > currentIndent) {
              const child: Record<string, unknown> = {};
              const endIdx = YamlAccessor.parseBlock(lines, nextLineIdx, nextIndent, child);
              nested[key] = child;
              target.push(nested);
              i = endIdx;
              continue;
